...
| **المصادقة** | Bearer Token (Laravel Sanctum – Personal Access Token) |
| **الترميز** | UTF-8 |
...
| `TOKEN_EXPIRED` | 401 | انتهت صلاحية access token |
...
```json
{
  ...
  "data": {
    ...
    "name": "أحمد",
    "email": "ahmad@example.com",
    "accessToken": "3|nQ7Xk2FvT8mZpL4wR1cYb3jH0sU5dEoAi6gK9",
    "refreshToken": "4|dR2mYpB8vX1qLtN6fW9cZ3jA5hS7uEoQi0kG4"
  }
}
```
...
```json
{
  "success": true,
  "data": {
    "accessToken": "5|kT9pXe2RfN8wL4jC6dYsU1aZ0oI5nHmQ7bV3",
    "refreshToken": "6|wA4jC6dYsU1aZ0oI5nHmQ7bV3kT9pXe2RfN8",
    "expiresIn": 3600
  }
}
```
...
### 6.7 دورة حياة المهمة الكاملة (مطابقة لشاشات التطبيق: الكل / دراسة / رياضة)

يوضّح هذا القسم — بمثال واحد متسلسل — كيف تُقرأ نفس المهمة عبر تبويبي **"الكل"** و**تصنيفها الخاص** (دراسة/رياضة) في الشاشة الرئيسية، دون أي جدول أو تخزين منفصل: مصدر الحقيقة الوحيد هو جدول `tasks`، والفلترة تتم فقط عبر معامل `type` في نفس `GET /tasks` (قسم 6.1).

**الخطوة 1 — إنشاء مهمة رياضية جديدة**
`POST /tasks`
```json
{ "title": "جري صباحي", "type": "sport", "dueDate": "2026-09-18", "scheduledTime": "07:00", "durationMinutes": 30, "priority": "medium" }
```
**استجابة (201)**
```json
{
...
**الخطوة 2 — تبويب "الكل"**: `GET /tasks` (بدون `type`) — المهمة الجديدة تظهر ضمن كل الأنواع فوراً لأنه لا يوجد فلتر مطبّق أصلاً على الاستعلام.

**الخطوة 3 — تبويب "رياضة"**: `GET /tasks?type=sport` — نفس المهمة تظهر أيضاً هنا لأن `type = "sport"` يطابق شرط الفلتر. **لا تُكتَب المهمة مرتين ولا يوجد جدول `sport_tasks` منفصل** — نفس السجل يُقرأ في كلا الاستعلامين، والفرق فقط في `WHERE type = ...`.
> لو كانت المهمة `type = "study"` بدل `sport`، تظهر بنفس المنطق في تبويب "الكل" وفي تبويب "دراسة"، وتختفي تلقائياً من تبويب "رياضة".

**الخطوة 4 — عرض التفاصيل**: `GET /tasks/{id}` (قسم 6.2) — يعيد نفس بنية العنصر أعلاه لعرضها في شاشة تفاصيل المهمة.

**الخطوة 5 — تعديل**: `PUT /tasks/{id}` (قسم 6.4) بجسم مثل:
```json
{ "title": "جري صباحي — 5 كم", "durationMinutes": 35 }
```
التغيير يظهر فوراً في كلا التبويبين ("الكل" و"رياضة") لأنه نفس السجل، وأي تغيير في `type` (مثلاً من `sport` إلى `other`) ينقل المهمة إلى تبويبها الجديد ويزيلها من القديم في نفس اللحظة.

**الخطوة 6 — حذف**: `DELETE /tasks/{id}` (قسم 6.6) — بعد الحذف (Soft Delete)، تختفي المهمة من كل الاستعلامات (`GET /tasks` بكل قيم `type`) وكذلك من `GET /tasks/{id}` الذي يعيد `404 TASK_NOT_FOUND`، إلى أن تُستعاد عبر المزامنة أو تُحذف نهائياً.
...
| POST | /auth/logout | تسجيل الخروج (إبطال access token الحالي) |
